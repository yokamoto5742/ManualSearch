...
1. [なぜ ManualSearch なのか](#なぜ-manualsearch-なのか)
2. [こんな場面で効く](#こんな場面で効く)
3. [主要機能](#主要機能)
4. [前提条件と要件](#前提条件と要件)
5. [インストール手順](#インストール手順)
...
「あのマニュアルのあの記載、どこに書いてあったか」を探し回る時間は、既存の検索ツールでは意外と減りません。ManualSearch は、その不満を一つずつ解消します。

| 既存ツールの不満 | ManualSearch の答え |
|---|---|
| **Everything**：ファイル名しか引けない | ファイル本文を全文検索 |
| **grep**：GUIがなく、PDFが読めない | GUI付き・PyMuPDFでPDF本文に対応 |
| **Acrobat**：1ファイルずつしか探せない | 複数フォルダを横断検索し、該当箇所を自動ハイライト |
| **Windows検索**：遅い・日本語が弱い | インデックスで高速・Shift_JIS/UTF-8を自動判定 |

そして検索対象に**共有フォルダ**を指定すれば、チーム全員が同じ資料を自分の端末から引けるようになります。

> 「あの人に聞かないと分からない」が「検索すれば誰でも引ける」に変わります。
