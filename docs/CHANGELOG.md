...
### 追加
- テキストビューア設定：config.ini にテキストビューア用設定セクションを追加し、表示カスタマイズに対応

### 変更
- テキスト表示機能：service/text_handler.py をリファクタリングし、テキスト・Markdown ファイルを独立したウィンドウで表示するよう改善
- テキストビューアウィジェット：widgets/text_viewer_widget.py として分離し、UI コンポーネントの再利用性を向上
