...
### 追加
- 未捕捉例外のログ記録（main.py）：アプリケーション内で処理されない例外が発生した際にログに記録し、予期しない終了を防止
- テキストビューアの「ファイルを開く」機能強化：Windows UNCパスに対応し、エラーハンドリングを追加

### 変更
- テキストビューアのファイルオープン処理（widgets/text_viewer_widget.py）：パスの正規化とエラーメッセージ表示を改善

### 修正
- テキストビューア印刷時のハイライト表示：クローン文書を使用し、検索ハイライトが印刷結果に含まれないよう修正
...
### 変更
- PDFハイライト（service/pdf_handler.py）：ハイライト注釈を画面表示のみの設定にし、Acrobatでの印刷結果にハイライトが含まれないよう変更
- ハイライト設定チェックボックス（widgets/directory_widget.py）：印刷には反映されない旨のツールチップを追加

### 削除
- 最後に使用したディレクトリ設定を削除（utils/config_manager.py）：起動時の前回使用ディレクトリの自動復元機能を廃止

## [1.4.2] - 2026-08-12

### 追加
- テキストビューア印刷機能：テキスト・Markdownの表示内容を印刷プレビュー付きで印刷するボタンを追加（ファイルを開くボタンと閉じるボタンの間に配置）
- 印刷エラー通知：プリンタ未接続や印刷失敗時に自動クローズメッセージでエラーを通知

## [1.4.1] - 2026-06-06

### 追加
- テキストビューアボタン：テキストビューアにファイルを開く・閉じるボタンを追加し、ファイル操作を改善

### 変更
- 検索メソッド（results_widget.py）：複数の検索メソッド（単一・グローバル・インデックス検索）を統合し、コード重複を削減
- ロギング機能：print文をloggingモジュールに置き換え、エラー追跡を強化
- エラーメッセージ管理（utils/constants/error.py）：ファイルオープン関連のエラーメッセージを定数化し、管理を改善
